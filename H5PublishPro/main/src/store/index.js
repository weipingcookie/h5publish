...
export default new Vuex.Store({
	state: {
		...
		defaultPageSize: 12, //每页显示12个页面
		fenyeData: '',
		catoryNavData: [{   //频道分类数据
				name: '国家大剧院',
				id: 101
			},
			{
	...
	mutations: {
		...
		deletePageItem(index) {   //删除当前编辑项
			this.state.pageData[0].data.splice(index, 1)
		},
		setActive(item, index) {  //模块分类当前点击项添加class
		...
		changeSearchForName(state, name) {  //根据标题显示页面
			state.searchForName = name
		},
		deleteCurrentModel(state, val) { //删除当前操作的模块
